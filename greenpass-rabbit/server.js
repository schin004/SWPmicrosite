...
function buildEdmHtml(hires, occasion, sendDate) {
  const GREEN = '#2D6A4F', SAGE_BG = '#E8F5E9', CREAM = '#F8F4E3', BROWN = '#6B4226';
  const rows = hires.map((h, i) => {
    const bg = i % 2 === 0 ? '#FFFFFF' : SAGE_BG;
    const img = photoDataUri(h);
    const photoCell = img
      ? `<img src="${img}" width="100" height="100" alt="${esc(h.full_name)}" style="width:100px;height:100px;border-radius:50%;object-fit:cover;display:block;border:3px solid ${GREEN};" />`
      : `<div style="width:100px;height:100px;border-radius:50%;background-color:${GREEN};"></div>`;
    const funFact = h.fun_fact ? `<p style="margin:10px 0 0 0;font-family:Arial,Helvetica,sans-serif;font-size:13px;color:${BROWN};font-style:italic;">🌱 Fun fact: ${esc(h.fun_fact)}</p>` : '';
    return `<tr style="background-color:${bg};"><td valign="top" width="130" style="padding:20px 16px 20px 24px;">${photoCell}</td>
      <td valign="top" style="padding:20px 24px 20px 0;">
        <p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:19px;font-weight:bold;color:${GREEN};">${esc(h.full_name)}</p>
        <p style="margin:4px 0 0 0;font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:bold;color:${BROWN};">${esc(h.job_title)} &middot; ${esc(h.division)}</p>
        <p style="margin:4px 0 0 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#6c757d;">Started ${esc(h.start_date)}</p>
        <p style="margin:12px 0 0 0;font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.5;color:#333333;">${esc(h.intro)}</p>
        ${funFact}</td></tr>`;
  }).join('\n');
  const dateLine = sendDate ? `<p style="margin:6px 0 0 0;font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#d8f3dc;">${esc(sendDate)}</p>` : '';
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:${CREAM};padding:24px 0;"><tr><td align="center">
  <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;background-color:#ffffff;border-radius:12px;overflow:hidden;">
    <tr><td style="background-color:${GREEN};padding:32px 24px;text-align:center;">
      ...
      <p style="margin:10px 0 0 0;font-family:Arial,Helvetica,sans-serif;font-size:16px;font-weight:bold;color:#d8f3dc;">${esc(occasion)}</p>${dateLine}</td></tr>
    <tr><td style="padding:24px;"><p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6;color:#333333;">We're delighted to welcome our newest colleagues to the NParks family! Please take a moment to get to know them below — we hope you'll give them a warm welcome as we keep growing together.</p></td></tr>
    <tr><td style="padding:0 24px 8px 24px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;border-radius:10px;overflow:hidden;border:1px solid #e0e0e0;">${rows}</table></td></tr>
    <tr><td style="background-color:${GREEN};padding:24px;text-align:center;">
      ...
      <p style="margin:10px 0 0 0;font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#95d5b2;">National Parks Board (NParks), Singapore &middot; Making Singapore our City in Nature</p></td></tr>
  </table></td></tr></table>`;
}
